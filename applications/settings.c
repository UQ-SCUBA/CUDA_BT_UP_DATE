/*
 ...
 Additional Copyright 2023 Zach Whitrock zjwhitrock@gmail.com
 */
#include <stddef.h>
#include <stdlib.h>
#include <stdio.h>
...
bool set_speeds (int index, const char *data) // Increased limit from 5000 to 9000
{
    int i;
    int num = sscanf (data, "%i", &i);
    if (num != 1)
    ...
    if (i < 1000 || i > 9000)
    {
        commands_printf ("out of range. (1000-9000)\n");
        return false;
    }
...
bool set_limits (int index, const char *data) // Increased limit from 23 to 60
{
    float x;
    int num = sscanf (data, "%f", &x);
    if (num != 1)
    ...
    if (x < 0.5 || x > 60)
    {
        commands_printf ("out of range. (0.5-60)\n");
        return false;
    }
...
bool set_battlevels (int index, const char *data) // Increased range to 20-60
{
    float x;
    int num = sscanf (data, "%f", &x);
    if (num != 1)
    ...
    if (x < 20.0 || x > 60.0)
    {
        commands_printf ("out of range. (20.0 - 60.0)\n");
        return false;
    }
